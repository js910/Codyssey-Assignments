...
#### 1. 이미지와 컨테이너의 분리
* **이미지**는 프로그램 실행에 필요한 환경을 고정해둔 고정된 파일입니다. (변하지 않음)
* **컨테이너**는 이미지를 기반으로 실행되는 격리된 프로세스입니다.
* 이 구조는 **개발 환경과 운영 환경을 일치**시키며, 배포의 신뢰성을 보장합니다.

#### 2. 격리된 실행 환경
* 컨테이너는 호스트 OS를 공유하지만, 파일과 네트워크는 격리되어 작동합니다.
* 이는 다른 버전의 서비스를 하나의 서버에서 **충돌 없이 독립적으로 운영**할 수 있습니다.

#### 3. 포트 매핑 및 데이터 영속성
* 컨테이너는 기본적으로 외부와 차단된 네트워크를 사용합니다. 따라서 **포트 매핑**을 통해 외부(내 컴퓨터)와 내부(컨테이너)를 연결해줘야 서비스가 작동합니다.
* **데이터 영속성**: 컨테이너는 언제든 교체/삭제될 수 있는 **휘발성**을 가집니다.
  따라서 **Volume**로 별도 저장하거나 **Bind Mount**로 외부와 실시간 연동시켜 데이터를 안전하게 유지합니다.

**결론적으로, Docker는 애플리케이션을 환경으로부터 독립시켜 안정적으로 실행시킬 수 있는 도구임을 확인했습니다.**
