# Mini NPU Simulator
<p>
   <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white">
   <img src="https://img.shields.io/badge/AI-NPU_Simulation-orange">
   <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
</p>

## 1. 프로젝트 개요
본 프로젝트는 인공지능 연산의 핵심인 **MAC(Multiply-Accumulate)** 연산을 구현한 시뮬레이터입니다. 컴퓨터가 시각적 형태를 인식하는 원리를 2차원 배열 연산을 통해 재현하며, 데이터 크기 증가에 따른 성능 분석과 수치 연산의 예외 처리를 중점적으로 다룹니다.

<br>

## 2. 실행 환경 및 실행 방법
- **실행 환경**: Python 3.12.13, Git 2.53.0
- **제약**: 외부 라이브러리(NumPy 등) 사용 금지, 표준 라이브러리만 활용
- **실행 방법**:
  ```bash
    git clone https://github.com/js910/Codyssey-Assignments
    cd Codyssey-Assignments/Assignment-3-NPU
    python main.py
    ```
- **메뉴 구성**:
    - `1`: 사용자 직접 입력 모드 (3x3 필터 및 패턴)
    - `2`: `data.json` 기반 자동 분석 및 성능 측정 모드
    - `3`: 프로그램 종료

<br>

## 3. 구현 요약
- **MAC 엔진**: 이중 반복문을 통해 요소별 곱셈 후 누적 합산을 수행하는 순수 파이썬 로직 구현
- **검증 시스템**: 입력 데이터의 행/열 개수를 연산 전 검사('all()' 함수 활용)하여 인덱스 에러 방지
- **자동 리포트**: 테스트 결과 요약(PASS/FAIL) 및 크기별 평균 연산 시간 자동 산출

<br>

## 4. 핵심 원리

### MAC(Multiply-Accumulate) 연산이란?
- **개요**: 두 배열을 겹쳐 같은 위치의 숫자끼리 곱하고, 그 결과를 모두 더하는 연산입니다. 
- **AI에서의 중요성**: 딥러닝 모델은 수억 번의 MAC 연산을 통해 이미지의 선, 면, 모양 특징을 추출합니다. CPU의 직렬 처리로는 이 속도를 감당하기 어려워, 수천 개의 MAC 연산을 동시에(병렬) 처리하는 **NPU(Neural Processing Unit)**가 탄생했습니다.
- **점수 계산 원리**: 입력 패턴과 필터가 일치할수록 곱셈의 합(유사도 점수)이 높게 나타납니다.

### 데이터 규칙 및 라벨 정규화
- **키 규칙 해석**: 'data.json'의 패턴 키('size_N_idx')에서 크기 정보 'N'을 추출해 해당 크기의 필터와 매칭합니다.
- **정규화 이유**: 데이터 제공처마다 정답을 '+', 'cross', 'x', 'X' 등 서로 다르게 표기할 수 있습니다. 이를 내부 표준인 'Cross'와 'X'로 통일해야 정확한 비교 판정이 가능합니다.

### 부동소수점 오차와 허용오차(Epsilon)
- **오차 발생 원인**: 컴퓨터는 실수를 2진수로 표현하는 과정에서 미세한 오차가 발생합니다. (예: 0.1 + 0.2 != 0.3)
- **Epsilon 정책**: 'abs(score_a - score_b) < 1e-9' 조건을 적용하여, 아주 미세한 수치 차이는 동점으로 간주함으로써 연산 정확도의 한계를 극복합니다.

<br>

## 5. 결과 리포트

### 실패(FAIL) 케이스 원인 분석
발생한 실패 원인을 다음과 같이 세 가지 관점으로 진단했습니다.

1. **데이터/스키마 문제**: 'size_3_2' 패턴과 같이 행렬의 열 개수가 일치하지 않는 데이터 결함입니다. 본 프로그램은 이를 사전에 검증하여 시스템 중단 없이 해당 케이스만 FAIL 처리하도록 설계되었습니다.
2. **로직 문제**: 특정 패턴의 모양이 정답 라벨('expected')과 실제 연산 결과가 일치하지 않는 경우입니다. 이는 데이터 자체의 라벨링 오류일 가능성이 높습니다.
3. **수치 비교 문제**: epsilon 범위를 벗어나는 아주 정교한 점수 차이로 인해 발생하는 판정 불일치입니다.

### 성능 표 해석
| 크기 (N x N) | 평균 시간 (ms) | 연산 횟수 (N^2) |
| :--- | :--- | :--- |
| 3 x 3 | [본인의 측정값] | 9 |
| 5 x 5 | [본인의 측정값] | 25 |
| 13 x 13 | [본인의 측정값] | 169 |
| 25 x 25 | [본인의 측정값] | 625 |

**[시간 복잡도 근거 분석]**
- MAC 연산은 'N'개의 행과 'N'개의 열을 가진 2차원 배열 전체를 훑어야 하므로 **이중 루프(Nested Loop)**가 필수적입니다.
- 연산 횟수는 패턴의 가로/세로 길이의 제곱('N^2')에 비례하여 증가합니다. 
- 실제 측정 결과에서도 'N'이 커질수록 시간이 제곱에 비례하여 늘어나는 추세를 보이며, 이는 이론적인 **O(N^2)** 복잡도와 일치함을 확인했습니다.





부동소수점 오차가 판정에 어떤 영향을 주는지, 그리고 허용오차(epsilon) 기반 비교 정책이 필요한 이유를 설명,
크기별 연산 시간을 측정하고, 패턴 크기 증가에 따른 시간 복잡도 O(N²)를 근거와 함께 설명
실패 케이스가 발생했을 때 원인을 “데이터/스키마 문제 vs 로직 문제 vs 수치 비교 문제”로 분리해 진단
필수